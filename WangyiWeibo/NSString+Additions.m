...
//  testMsn
//
//  Created by zhikun wu on 6/15/11.
...
- (NSComparisonResult)compareEx:(NSString *)aString
{
    NSMutableString *string1 = [NSMutableString string];
    NSMutableString *string2 = [NSMutableString string];
    
    for (int i=0; i<[self length]; i++) {
        [string1 appendFormat:@"%c", pinyinFirstLetter([self characterAtIndex:i])];
    }
    for (int i=0; i<[aString length]; i++) {
        [string2 appendFormat:@"%c", pinyinFirstLetter([aString characterAtIndex:i])];
    }
