...
//    ASIHTTPRequest *ASIHttp = [[ASIHTTPRequest alloc] init];
    ASIHTTPRequest *asiRequest;
	NSURLConnection *connection;
	if ([aHttpMethod isEqualToString:@"GET"]) {
//		connection = [http httpGet:oauthUrl queryString:queryString delegate:aDelegate];
        asiRequest = [http ASIHttpGet:oauthUrl queryString:queryString delegate:aDelegate];
        return asiRequest;
	} else if (!aFiles || [aFiles count] == 0) {
...
+ (ASIHTTPRequest *)asiAsyncRequestWithUrl:(NSString *)aUrl
                              httpMethod:(NSString *)aHttpMethod
                                oauthKey:(MicroBlogOauthKey *)aOauthKey
                              parameters:(NSDictionary *)aParameters
                                   files:(NSDictionary *)aFiles
...
//    NSURLConnection *connection;
    if ([aHttpMethod isEqualToString:@"GET"]) {
        ...
        asiRequest = [http ASIHttpPost:oauthUrl queryString:queryString delegate:aDelegate];
    } else {
        asiRequest = [http ASIHttpPostWithFile:aFiles url:oauthUrl queryString:queryString delegate:aDelegate];
    }
